...
async function checkWord(){
    //turn current attempt into just a number for easy evaluation and incrimenting
    ...
    let winCount
    //First we check to be sure the work is long enough...
    if(enteredWord.length === 5){
        ...
        try{ 
            ...
            if(result[0]){ 
                //if it is a word we send it to our win loop to see if it's the winning word
                winCount = enterLoop()
...
function enterLoop(){
    ...
    //will keep track of the correctlky guessed and placed letters, if it gets to five they win
    let winner = 0
    //get the row we are one
    ...
    for(let i = 0; i<spacesToCheck.length; i++){
       ...
       let letterToCheck = spacesToCheck[i].textContent
        ...
        let color
        //checks the amount of thimes a letter appears in a word
        // these variable will depend on the row where the letter is on the keyboard since there are three different keyboard arrays and are responsible for finding the keys on the keyboard that need to change colors
        let keyboardPlace
        let keyboardRow
        //first row
       ...
       for(let x = 0; x <newWordArray.length; x++){
        //get each letter from the new word array
            ...
            //if the letter that we are on in spaces to check loop is eqaul to the letter we are on in the new word loop
       ...
       //if the color returned is green...
       if(color === "#69b969"){
        // add 1 to winner, if this is five at the end of the loop, the player has found the word. 
            winner++
       }
